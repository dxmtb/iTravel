...
Graph_Time::Graph_Time(const vector<const Train*> &trainList)
{
...
void Graph_Time::addEdge(const ArriveEvent &e1, const ArriveEvent &e2)
{
    Edge now = genEdge(e1, e2);
    edges[now.fromId].push_back(now);
}

vector<const Edge*> Graph_Time::getShortest(int S, int T, int change_limit, int late, int earliest, int latest, double rate) const
{
    int N = (int)Station::sta_list.size(), best = oo;
    ...
    for(vector<Edge>::const_iterator it_begin = edges[S].begin(); it_begin != edges[S].end(); it_begin++){
        ...
        {
            vector<pair<int,Time> > distance(N);
            vector<bool> sure(N);
            ...
            while(!sure[T])
            {
                ...
                if (near == -1 || distance[near].first >= best)
                    break;
                ...
                for(vector<Edge>::const_iterator it = edges[near].begin(); it != edges[near].end(); it++)
                    if ((near != S || it == it_begin) && !sure[it->toId])
                    {
                        ...
                        if (near != S && path[near]->e1->train != it->e1->train && inter < late)
                            continue;
                        int nd = distance[near].first + inter + it->interval;
            ...
            bool tmp = sure[T];
            int s_size = sure.size();
            int dt = distance[T].first;
